...
type EnvForInvite = {
  ...
  title: string;
  message: string | null;
  org: { id: string; name: string; brandColor: string | null; fromEmail: string | null };
};
    ...
    if (gate.ok) {
      try {
        emailed = await sendSigningInvite({
          ...
          orgName: env.org.name, brandColor: env.org.brandColor ?? undefined,
...
export async function advanceSequence(envelopeId: string): Promise<void> {
  const env = await db.envelope.findUnique({ where: { id: envelopeId }, include: { org: true, signers: true } });
  ...
  for (const s of toInvite) {
    await inviteSigner(env, s as SignerRow);
