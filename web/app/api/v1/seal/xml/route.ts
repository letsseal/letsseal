...
import { hostedSealXml } from "@/lib/hosted";
...
export async function POST(req: NextRequest) {
  ...
  if (!(file instanceof File)) {
    return NextResponse.json({ error: "multipart form with an XML 'file' field required" }, { status: 400 });
  }
  ...
  try {
    const r = await hostedSealXml(auth.ctx.org, xml, { filename: file.name, title, anchor: doAnchor });
    return new Response(new Uint8Array(r.xml), {
      headers: {
        "Content-Type": "application/xml",
        "Content-Disposition": 'attachment; filename="sealed.xml"',
    ...
    return NextResponse.json({ error: `xml seal failed: ${e instanceof Error ? e.message : e}` }, { status: 502 });
  }
}
