...
import { hostedSealDetached } from "@/lib/hosted";
...
export async function POST(req: NextRequest) {
  ...
  if (!sha256 || !/^[0-9a-f]{64}$/.test(sha256)) {
    return NextResponse.json(
      { error: "provide a 'sha256' (64 hex) to seal digest-only, or a 'file' to hash server-side" },
      { status: 400 },
    );
  }
  ...
  try {
    const r = await hostedSealDetached(auth.ctx.org, sha256, { title, anchor: doAnchor });
    return NextResponse.json(
      { sha256: r.sha256, sig: r.sig, certCN: r.certCN, anchorState: r.anchorState, proofUrl: r.proofUrl, proofCode: r.proofCode },
      { headers: { "Cache-Control": "no-store" } },
    );
  } catch (e) {
    return NextResponse.json({ error: `detached seal failed: ${e instanceof Error ? e.message : e}` }, { status: 502 });
  }
}
