...
export async function POST(req: NextRequest) {
  ...
  if (ctype.includes("application/json")) {
    ...
    if (body.provider != null) provider = String(body.provider);
    if (body.token != null) token = String(body.token);
    ...
    if (form?.get("provider")) provider = String(form.get("provider"));
    if (form?.get("token")) token = String(form.get("token"));
  ...
  if (!provider || !/^[a-z0-9][a-z0-9-]{0,30}$/.test(provider)) {
    return NextResponse.json({ error: "provide a 'provider' (e.g. google, github)" }, { status: 400 });
  }
  if (!token || token.length < 8) {
    return NextResponse.json({ error: "provide a 'token' (the provider's OIDC/OAuth proof)" }, { status: 400 });
  }
  const doAnchor = anchorRaw == null ? true : !/^(false|0|no)$/i.test(anchorRaw);

  try {
    const r = await hostedSealIdentity(auth.ctx.org, sha256, provider, token, { title, anchor: doAnchor });
    return NextResponse.json(
      {
        sha256: r.sha256, sig: r.sig, certPem: r.certPem, chainPem: r.chainPem,
        identity: r.identity, issuer: r.issuer, provider: r.provider,
        anchorState: r.anchorState, proofUrl: r.proofUrl, proofCode: r.proofCode,
      },
      { headers: { "Cache-Control": "no-store" } },
    );
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    const status = /\b401\b|did not verify/.test(msg) ? 401 : 502;
    return NextResponse.json({ error: `identity seal failed: ${msg}` }, { status });
  }
}
