...
C-59 fixes the verdict vocabulary a conforming verifier reports: **authentic**,
**unrecognised** and **altered**, with anchor state reported separately as **confirmed** or
**pending**. This suite reports more than that, because several vectors need a finer answer:
`revoked`, `not_sealed`, the anchor states `absent` and `mismatch`, the `reason` vocabulary
of §1.1, `issuerVerified`, and the `revocation` object. Those are proposed amendments to
SPEC.md and CONFORMANCE.md, recorded in [§7](#7-open-questions-and-work-items), and a verifier
that reports the C-59 vocabulary alone conforms today. A harness compares those fields for an
implementation that reports them and records them as not applicable for one that does not
(§5).
...
| `trusted` | boolean | The signing certificate path-validates to the pinned SEAL root, is within its validity window at the evaluation time, and carries no blocking revocation (§4.9 of CPS.md). |
| `coverage` | enum | What the signature covers: `entire_file`, `contiguous_block_from_start`, `entire_revision`, `other`, `none`. |
| `authentic` | boolean | `intact ∧ valid ∧ trusted ∧ coverage == "entire_file"`, evaluated on an artifact where `sealed` holds. SPEC.md §8 step 4 names valid, trusted and entire-file; its closing rule adds `intact` and requires that `sealed` or `intact` alone never render a pass. CONFORMANCE.md states the same pair as C-6 and C-8. `spec/verify.py` computes exactly this four-term conjunction, and `manifest.json` records it. |
| `verdict` | enum | The single word a user interface shows: `authentic`, `altered`, `unrecognised`, `revoked`, `not_sealed`. |
| `anchorState` | enum | `confirmed`, `pending`, `absent`, `mismatch`, `unknown`. |
...
| `revocation` | object or null | `{ "listed": true, "reason": string, "effect": "unconditional"\|"time_bounded"\|"none", "appliesToThisSeal": bool }`, with an optional `revokedAt`. The object is present exactly when the signing certificate appears in the revocation list, whatever the outcome, so `listed` is always `true` where the object exists and `null` carries the other case. `revokedAt` is carried where the harness has the list in hand and is omitted from the expectations in §3, which assert the reading rather than the date. |
...
1. `sealed == false` → `not_sealed`
2. `intact == false` **or** `valid == false` **or** `coverage != "entire_file"` → `altered`
3. `trusted == false` because a blocking revocation applies → `revoked`
4. `trusted == false` for any other reason (the chain does not reach the pinned root, the
   chain is incomplete, the certificate is outside its validity window) → `unrecognised`
5. otherwise → `authentic`

The `unrecognised` wording is normative in SPEC.md §8: a valid signature from a certificate
outside the pinned root is a forgery vector and is reported as unrecognised.

The reference verifier `spec/verify.py` uses this order: it prints NOT A SEAL where no
signature is present, then tests `intact`, then `valid`, then coverage, and reaches
unrecognised only after all three hold. Vector 002 exercises the
...
### 1.3 The two rules that carry the suite

**Authenticity is a conjunction.** `authentic` requires all four terms. Rendering a pass
from `sealed` and `intact` alone is the failure mode SPEC.md §8 and CONFORMANCE.md C-8 exist
to prevent, and vector 003 is the one that catches it.

**Time is a separate axis.** `anchorState` never contributes to `authentic`. The anchor is
independent evidence of *when*, and it also decides how far a time-bounded revocation
reaches (vectors 010 and 011). A verifier keeps the two axes apart in both its logic and
its display.

### 1.4 Evaluation time

Certificate validity and time-bounded revocation are evaluated at a moment. The manifest
names it per vector:

- `evaluateAt: "now"` (default): the harness's wall clock.
- `evaluateAt: "anchorTime"`: the block time proven by a confirmed anchor.

Where a vector's answer differs between the two, the manifest carries a second expectation
block, `expectAtAnchorTime`, asserted only by a harness that supports time-bound
evaluation. `spec/verify.py` supports it through `--attime`.
...
thing the suite exists to do. The revocation vectors additionally wait on the reference
verifier reading a revocation list from disk (§7 item 10). They land when they can be made
honestly, and until then the suite says which sections of SPEC.md it covers: §2 and §8.
...
**Teaches:** SPEC.md §8 step 4. Validity and trust are separate terms, and authenticity is
the conjunction.
...
**Teaches:** the rule in SPEC.md §2 that a PDF signature covering part of a file is
reported as altered. Every other field here is green. Coverage alone decides.
...
the digest of the whole file. Byte level disagreement surfaces in `intact`.
...
reports the field differences and passes the vector. §7 item 4 carries the fix to the
reference verifier that removes the second one.
...
on `unknown` instead: `spec/verify.py` maps anything neither success-like nor pending-like
to `unknown`. Where the condition cannot be met, the vector ships
...
`signing-service/revocation.py` reads and `ca/setup-ca.sh revoke` writes:
...
```json
{ "sealed": true, "intact": true, "valid": true, "trusted": false,
  "coverage": "entire_file", "authentic": false, "verdict": "revoked",
  "anchorState": "confirmed", "reason": "certificate_revoked", "issuerVerified": false,
  "revocation": { "listed": true, "reason": "key_compromise",
                  "effect": "unconditional", "appliesToThisSeal": true } }
```

**Teaches:** CPS.md §4.9. A compromised key was in another party's hands from a moment
nobody can establish, so every seal under it falls, whatever its date and however good its
evidence of time.
...
{ "sealed": true, "intact": true, "valid": true, "trusted": true,
  "coverage": "entire_file", "authentic": true, "verdict": "authentic",
  "anchorState": "confirmed", "reason": null, "issuerVerified": true,
...
seal stands, which is the honest presentation.
...
{ "sealed": true, "intact": true, "valid": true, "trusted": false,
  "coverage": "entire_file", "authentic": false, "verdict": "revoked",
  "anchorState": "absent", "reason": "certificate_revoked", "issuerVerified": false,
  "revocation": { "listed": true, "reason": "superseded",
...
{ "sealed": false, "intact": false, "valid": false, "trusted": false,
  "coverage": "none", "authentic": false, "verdict": "not_sealed",
...
At wall-clock time (`evaluateAt: "now"`):

```json
{ "sealed": true, "intact": true, "valid": true, "trusted": false,
  "coverage": "entire_file", "authentic": false, "verdict": "unrecognised",
  "anchorState": "confirmed", "reason": "certificate_expired", "revocation": null }
```

`expectAtAnchorTime` (asserted by a harness supporting time-bound evaluation):
...
**Teaches:** the same artifact yields two correct answers, and the verifier states which
moment it evaluated at. `spec/verify.py` prints exactly that line.
**Catches:** the blanket time bypass. A verifier that disables validity checking to stop
old documents failing then accepts a leaked leaf forever, and a verifier that always uses
wall-clock time reports every properly sealed five year old document as untrustworthy.
...
{ "sealed": true, "intact": true, "valid": true, "trusted": true,
  "coverage": "entire_file", "authentic": true, "verdict": "authentic",
  "anchorState": "mismatch", "reason": null, "revocation": null }
```
...
coverage `entire_file` from the hard binding over the whole asset.
...
embedded. The vector carries `sealType: "smime"`, and its `expect` block mirrors 001.
Verifies with `openssl smime -verify -in message.eml -CAfile roots/seal-test-root.pem`.
...
**Files:** `leaf.json`, `proof.json`, `sth.json`, `log-key.pem`, `notes.md`
...
```json
{ "inclusion": true, "treeSize": 128, "rootHashMatches": true, "sthSignatureValid": true }
```
...
vector is the only practical way to find them. SPEC.md §6 writes the leaf without the `v`
field and calls the serialisation "canonical-JSON"; §7 item 7 carries both points.
  ...
  "$defs": {
    ...
    "expectation": {
      ...
      "properties": {
        ...
        "verdict":  { "enum": ["authentic", "altered", "unrecognised",
                               "revoked", "not_sealed"] },
        "anchorState": { "enum": ["confirmed", "pending", "absent",
                                  "mismatch", "unknown"] },
        "reason": {
          "oneOf": [
            { "type": "null" },
    ...
    "logExpectation": {
      "type": "object",
      "required": ["inclusion", "treeSize", "rootHashMatches", "sthSignatureValid"],
      "additionalProperties": false,
      "properties": {
    ...
    "vector": {
      ...
      "properties": {
        ...
        "expectAtAnchorTime":{ "$ref": "#/$defs/expectation" },
...
```json
{
  ...
  "teaches": "SPEC.md section 8 step 4: validity and trust are independent terms, and authenticity is the conjunction of both with entire-file coverage. The signature here is cryptographically perfect and the certificate chain is internally consistent; it simply terminates at a root the verifier was never given.",
...
4. Load `revocationList` into the verifier, keyed by certificate serial, with the reason
   semantics of CPS.md §4.9. A verifier with no revocation input skips vectors 009 to 011
   and says so.
...
- A field outside the C-59 vocabulary that the verifier under test does not report
  (`issuerVerified`, `revocation`, `reason`, and the `verdict` and `anchorState` values §7
  proposes) is recorded as not applicable, distinct from PASS and from SKIP. Conformance
  rests on C-59's vocabulary; the rest is reported so a richer implementation gets checked.
- When `requiresNetwork` is true and the ledger is unreachable, `anchorState` reports SKIP
  and every other field is still asserted.
...
- **Revocation.** `spec/verify.py` has `--check-revocation`, which turns on online CRL and
  OCSP against endpoints this CA does not publish. It reads no revocation list from disk, so
  vectors 009, 010 and 011 and every `revocation` object and `verdict: "revoked"` are out of
  its reach until it ingests `revocations.json` with the semantics of
  `signing-service/revocation.py`.
...
- **Structured output.** It prints a `RESULT` line rather than emitting `verdict`, `reason`,
  `issuerVerified` or an `anchorState` of `absent` or `mismatch`. The §1.1 vocabulary needs a
  machine-readable form before a harness can compare it.
...
for SPEC.md, CPS.md and CONFORMANCE.md; items 10 to 13 are work this design depends on.
...
2. **Verdict precedence.** SPEC.md §8 defines `authentic` as a conjunction and names
   `unrecognised` for the untrusted-chain case, and it leaves the ordering open when several
   terms fail at once. §1.2 above fixes an order for the suite; §8 is the right home for it.
...
3. **`unspecified` as a revocation reason.** `signing-service/revocation.py` places
   ...
   falls through the "a reason a verifier does not recognise" row. CPS.md §4.9 names it in
   neither row. The behaviour is right and strict; §4.9 should name `unspecified` in the
   unconditional row so an implementer reads it once.

4. **Detached coverage semantics.** `spec/verify.py` returns `entire_file` as a copy of
   `valid` for detached seals, so vector 006 reports `entire_file: false` where this design
   says the signature covers the whole file by construction and `intact` carries the byte
   mismatch. Verdicts agree either way, and the field-level divergence is why 006 marks both
   `trusted` and `coverage` soft. Reporting coverage independently of `valid` on the
   detached path closes it.

5. **Anchor state vocabulary.** `spec/verify.py` returns `no-ots-client` and `error`
   alongside `confirmed` and `pending`. The manifest enum uses `unknown` for both, plus
   `absent` and `mismatch`, which §3 of SPEC.md does not currently name. §3 is the right
   place to fix the vocabulary.

6. **Digest binding of the anchor.** SPEC.md §3 says the `.ots` commits to the SHA-256 of
   the sealed document, and it stops short of requiring a verifier to compare that committed
   digest against the file in hand. Vector 015 tests that comparison. §8 step 3 should
   require it.

7. **The leaf preimage in SPEC.md §6.** `web/lib/translog.ts` emits
   `{"v":1,"sha256":…,"sealType":…,"certCN":…,"ts":…}` and SPEC.md §6 writes the leaf as
   `canonical-JSON{sha256, sealType, certCN, ts}`, omitting the `v` field the running log
   has emitted since the log was built. An implementer following §6 computes leaf hashes
   that match nothing. §6 should carry the `v` field, and should say that key order is fixed
   in the order given rather than sorted, since RFC 8785 JCS sorts and would change every
   leaf hash. This is the concrete answer to CONFORMANCE.md open point 7, which proposes JCS.

8. **Reporting beyond CONFORMANCE.md C-59.** C-59 requires the vocabulary authentic /
   unrecognised / altered, with anchor state confirmed or pending. This suite reports
   `revoked`, `not_sealed`, the anchor states `absent`, `mismatch` and `unknown`, the §1.1
   `reason` vocabulary, `issuerVerified`, and the `revocation` object. Each is proposed as an
   amendment: `revoked` and the revocation object to SPEC.md §8 and CONFORMANCE.md §1,
   `not_sealed` to close CONFORMANCE.md open point 11, the anchor states to SPEC.md §3
   (open point 10), and `issuerVerified` to SPEC.md §2 alongside C-11. Until they land, a
   verifier reporting the C-59 vocabulary alone conforms, and the harness records the rest as
   not applicable (§5).
...
10. **Reference verifier work items.** Revocation-list ingestion from disk, lowercase
    coverage names, and structured `verdict` / `reason` / `issuerVerified` output, as §5
    sets out. Vectors 009 to 011 wait on the first of the three.
