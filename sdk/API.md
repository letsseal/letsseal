...
**Seal forms at a glance.** One seal, in the right shape for each file type — every
...
Digest-only forms take a SHA-256 you compute locally — the file's bytes never leave
...
JSON `{ "sha256": "<64 hex>", "org_slug": "acme" }`. Seals **any file** — the file
...
`.pem` — the chain is completed server-side, so leaf-only is enough). Confirms the
...
root, the public transparency log, and the Bitcoin anchor — everything a verifier
